...
music: list = glob.glob(f"{Path.cwd() / 'track' / '*.mp3'}")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-f', '--frame', default=10, type=int)
...
if __name__ == '__main__':
    ...
    for i in range(namespace.count):
        subprocess.call(
            ...
            "{Path.cwd() / "track" / f"tmp_{i}.mp3"}" -y -loglevel quiet'
            )
        if namespace.log:
            print('file {0} has completed'.format(music[i][music[i].rfind("\\")+1:]))

    tmps: list = glob.glob(f"{Path.cwd() / 'track' / 'tmp*.mp3'}")
    ...
    if namespace.extended:
        for i, tmp in enumerate(tmps):
            subprocess.call(
                ...
                "{Path.cwd() / "track" / f"tmp_new{i}.mp3"}" -y -loglevel quiet'
                )
            remove(tmp)
        tmps: list = glob.glob(f"{Path.cwd() / 'track' / 'tmp_new*.mp3'}")

    with open(f'{Path.cwd() / "track" / "files.txt"}', 'w') as f:
        f.writelines(f"file '{tmp}'\n" for tmp in tmps)

    subprocess.call(
        f'ffmpeg -f concat -safe 0 -i \
        "{Path.cwd() / "track" / "files.txt"}" -c copy \
        "{Path.cwd() / "track" / f"{namespace.destination}"}" -y -loglevel quiet'
        )

    if namespace.log and path.exists(Path.cwd() / "track" / f"{namespace.destination}"):
        print(f'file {namespace.destination} has created')

    remove(Path.cwd() / "track" / "files.txt")
    for tmp in tmps:
        remove(tmp)
